...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='For auto-moderation';
...
CREATE TABLE `fmusers` (
  ...
  `LastUpdated` int(11) NOT NULL COMMENT 'The date and time this was last set',
  PRIMARY KEY (`User`),
...
CREATE TABLE `golds` (
  ...
  `User` bigint(20) unsigned DEFAULT NULL COMMENT 'The user given gold',
  `TimeGiven` int(10) unsigned DEFAULT NULL COMMENT 'The time they were given gold',
  `GivenBy` bigint(20) unsigned DEFAULT NULL COMMENT 'The user they were given gold by',
  `Message` bigint(20) DEFAULT NULL COMMENT 'The message they were given gold for',
  PRIMARY KEY (`ID`),
...
CREATE TABLE `nicknamechanges` (
  `ID` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `User` bigint(20) unsigned NOT NULL,
  `Old` varchar(38) DEFAULT NULL,
  `New` varchar(38) NOT NULL,
  `Time` int(10) unsigned NOT NULL,
  PRIMARY KEY (`ID`),
  KEY `fkIdx_883` (`User`),
...
CREATE TABLE `purchasedroles` (
  ...
  `Active` tinyint(1) NOT NULL COMMENT 'Whether or not the user has the role applied',
  PRIMARY KEY (`ID`),
...
CREATE TABLE `tags` (
  ...
  `Uses` int(11) GENERATED ALWAYS AS (0) VIRTUAL COMMENT 'The amount of time the tag has been used',
  PRIMARY KEY (`TagName`),
...
CREATE TABLE `transactions` (
  ...
  `PreviousBalance` int(11) NOT NULL COMMENT 'The balance before the transaction',
  `NewBalance` int(11) NOT NULL COMMENT 'The balance after the transaction',
  PRIMARY KEY (`ID`),
...
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COMMENT='Storage for any change in credits';
...
CREATE TABLE `transactiontypes` (
  ...
  `Name` varchar(50) NOT NULL COMMENT 'The type name',
  PRIMARY KEY (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COMMENT='Stores the types of transactions (ex, daily use, level up, role purchase, etc)';


CREATE TABLE `usernamechanges` (
  `ID` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `User` bigint(20) unsigned NOT NULL,
  `Old` varchar(38) NOT NULL,
  `New` varchar(38) NOT NULL,
  `Time` int(10) unsigned NOT NULL,
  PRIMARY KEY (`ID`),
...
CREATE TABLE `users` (
  ...
  `ActiveUser` tinyint(1) NOT NULL DEFAULT 1 COMMENT 'If they still are in the server',
  PRIMARY KEY (`ID`)
...
CREATE TABLE `joins` (
  ...
  KEY `fkIdx_869` (`User`),
  CONSTRAINT `FK_868` FOREIGN KEY (`User`) REFERENCES `users` (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8mb4 COMMENT='Stores records of users joining the server';
...
CREATE TABLE `leaves` (
  ...
  KEY `fkIdx_876` (`User`),
  CONSTRAINT `FK_875` FOREIGN KEY (`User`) REFERENCES `users` (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=2 DEFAULT CHARSET=utf8mb4 COMMENT='Stores records of users leaving the server';
...
CREATE TABLE `nicknamechanges` (
  ...
  KEY `fkIdx_883` (`User`),
  CONSTRAINT `FK_882` FOREIGN KEY (`User`) REFERENCES `users` (`ID`)
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8mb4 COMMENT='Stores nickname changes';
